...
Constants.GET_COMMUNITY_FOR_USER_QUERY = "SELECT CommunityName, CommunityID FROM " + Constants.DB_SCHEMA.TABLES.COMMUNITIES + " WHERE communityID IN (SELECT community_id FROM " + Constants.DB_SCHEMA.TABLES.COMMUNITYUSERS + " WHERE `user_id`=(SELECT UserID from " + Constants.DB_SCHEMA.TABLES.USERS + " WHERE email=?))";
...
Constants.GET_POSTS_IN_COMMUNITY_QUERY = "SELECT * FROM " + Constants.DB_SCHEMA.TABLES.POSTS + " WHERE ParentCommunity = ?";
